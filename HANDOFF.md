...
最后更新：2026-04-20（账号池已修复同邮箱 free/team 导入互相覆盖，并在扩展/桌面端明确显示 FREE/TEAM 标签；账号池表去掉套餐列并合并 5h/7d 用量列；扩展升到 1.0.5；桌面版仍停在 1.1.0 本地 unsigned 产物）
...
- 账号类型已经由 badge 直接表达
- 表格主信息密度更高
- 左侧栏窄宽度下更容易看全，不必额外拉宽面板

同时顺手把账号池表的最小宽度从 `760px` 收到了 `680px`，让缩窄场景下横向滚动更少。
